::Stronghold.LocationDefs <-
{
	Workshop = {
		...
		ConstID = "Workshop",
		Price = 0, //cost is set below this array
...
{
	location.Price = ::Stronghold.Locations[locationID].Price;
	location.MaxAmount <- ::Stronghold.Locations[locationID].MaxAmount;
	location.Requirements.push({
		Text = "Price: " + location.Price * ::Stronghold.PriceMult,
		IsValid = @(_town) this.World.Assets.getMoney() >= ::Stronghold.Locations[locationID].Price * ::Stronghold.PriceMult
	})
}
