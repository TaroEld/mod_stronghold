...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
	...
	{
		return {
			Text = "Build a" + (_screenVar.Name[0] == "A" ? "n ":" ") + _screenVar.Name + " (" +  (_screenVar.Price * this.Stronghold.Misc.PriceMult) + " crowns)",
			function getResult(_option)
			{
				...
				this.Contract.setCost(building.Price * this.Stronghold.Misc.PriceMult)
	...
	function addLocationScreen(_screenVar, _idx)
	{
		...
		local title = format("Build %s %s (%i crowns)", isInVocals(_screenVar.Name[0]) ? "an " : "a ", _screenVar.Name, _screenVar.Price * this.Stronghold.Misc.PriceMult);
		return {
			Text = title,
			function getResult(_option)
			{
	...
	{
		local price = _screenVar.Price * this.Stronghold.Misc.RoadCost * this.Stronghold.Misc.PriceMult
		return{
			Text = format("Road to %s (%i Crowns)", _screenVar.Name, price)
			function getResult(_option)
			{
				this.Contract.m.Temp_Var <- this.Option;
				this.Contract.setCost(this.Option.Price * this.Stronghold.Misc.RoadCost * this.Stronghold.Misc.PriceMult)
				this.Contract.addOverviewScreen(
					format("Build a road"), 
